...
namespace JFlacLib.JFlac.Metadata
{
    ...
    {
        private const int CuesheetMediaCatalogNumberLen = 128*8; // bits
        private const int CuesheetLeadInLen = 64; // bits
        private const int CuesheetIsCdLen = 1; // bit
        private const int CuesheetReservedLen = 7 + 258*8; // bits
        private const int CuesheetNumTracksLen = 8; // bits
        ...
        public CueSheet(BitInputStream inputStream, int length, bool isLast)
            : base(isLast)
        {
            inputStream.ReadByteBlockAlignedNoCRC(mediaCatalogNumber, CuesheetMediaCatalogNumberLen/8);
            leadIn = inputStream.ReadRawULong(CuesheetLeadInLen);
            isCD = (inputStream.ReadRawUInt(CuesheetIsCdLen) != 0);
            inputStream.SkipBitsNoCRC(CuesheetReservedLen);
            numTracks = inputStream.ReadRawUInt(CuesheetNumTracksLen);
