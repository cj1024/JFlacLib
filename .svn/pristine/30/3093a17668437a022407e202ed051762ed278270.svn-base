...
namespace JFlacLib.JFlac.Metadata
{
    public class VorbisComment : Metadata
    {
        ...
        public String[] GetCommentByName(String key)
        {
            ...
            return (from t in Comments select t.ToString() into comment let eqpos = comment.IndexOf((char) 0x3D) where eqpos != -1 where comment.Substring(0, eqpos).Equals(key, StringComparison.OrdinalIgnoreCase) select comment.Substring(eqpos + 1, comment.Length)).ToArray();
            //return null;
        }
    }
}
