...
namespace JFlacLib.JFlac.Frame
{
    public class ChannelFixed : Channel
    {
        ...
        private readonly int[] _warmup = new int[MaxFixedOrder]; // Warmup samples to prime the predictor, length == order.
        internal int[] Residual; // The residual signal, length == (blocksize minus order) samples.

        public ChannelFixed(BitInputStream inputStream, Header header,ref ChannelData channelData, int bps, int wastedBits,
                            int order)
