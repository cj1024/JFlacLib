...
{
    public class ChannelLPC : Channel
    {
        private const int SubframeLPCQlpCoeffPrecisionLen = 4; /* bits */
        private const int SubframeLPCQlpShiftLen = 5; /* bits */
        ...
        private readonly int[] _qlpCoeff = new int[MaxLPCOrder]; // FIR filter coefficients.

        private readonly int[] _warmup = new int[MaxLPCOrder];
        ...
        public ChannelLPC(BitInputStream inputStream, Header header, ref ChannelData channelData, int bps, int wastedBits,
                          int order)
            : base(header, wastedBits)
        {
            ...
            int u32 = inputStream.ReadRawUInt(SubframeLPCQlpCoeffPrecisionLen);
            if (u32 == (1 << SubframeLPCQlpCoeffPrecisionLen) - 1)
            {
            ...
            // read qlp shift
            QuantizationLevel = inputStream.ReadRawInt(SubframeLPCQlpShiftLen);
            ...
            var residual = channelData.Residual;
